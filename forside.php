    ...
    <ol data-role="listview" ><!--liste-->
      <?php              
        ...
        for($i=0, $lng=count($ar); $i<$lng; $i++){          
          ...
          for($k=0; $k<$lngk; $k++){
            $str.='<li class="swipe"><a href="opgaver.php?opgave='.$arr[$k]['id'].'" rel="external">'.$arr[$k]['navn'].'</a></li>'; // id = opgave_id
            $str.="\n";
          }          
        }         
        echo $str;   
     ...
     <ol data-role="listview" ><!--liste-->
     <li data-role="list-divider">Færdig opgaver</li>
      <?php              
        /*
        $ar = ReadAll($conn, "tbl_kategori");
        $str="";        
        for($i=0, $lng=count($ar); $i<$lng; $i++){          
          $arr = ReadMyOpenTaskInCategory1($conn, $_SESSION['USER']['id'], $ar[$i]['id']);
          $lngk=count($arr);
          // visser kun kategorier der er noget i
